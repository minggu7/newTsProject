...
// 게시글 작성(파일과 여러 데이터를 포함한 formData 형태로 전송함.)
//그래서 multipart 형태로 보내고. 받는데서는 multer 미들웨어 사용해서 파싱함.
export const createPost = async (formData: FormData) => {
  const response = await api.post('/postsCreate', formData, {
    headers: {
...
// 게시글 상세보기(첨부파일만 조회)
export const getPostDetail = async (myBoardPk: number): Promise<Attachment[]> => {
  const response = await api.post('/postDetail', {myBoardPk});
  console.log('response 데이터는? :', response.data);
...
// 파일 다운로드
//Blob 타입으로 받아야함.
export const downloadFile = async (uuid: string): Promise<Blob> => {
  const response = await api.get(`/download/${uuid}`, {
    responseType: 'blob'//blob으로 받겠다.(Axios에게 "응답 Blob으로 변환해줘" 라고 말해줌)
  });
  return response.data;
}